...
def encrypt():
    ...
    pub_file = request.files['public_key']

    if isinstance(message, str):
        message = message.encode()
    public_key = load_public_key(pub_file.stream)    

    input_path = os.path.join(UPLOAD_FOLDER, "input.png")
    image.save(input_path)

    eph_pub_bytes, iv, ciphertext = encrypt_data(public_key, message)
    ...
    filename = f"encrypted_{uuid.uuid4().hex}.png"
    output_path = os.path.join(UPLOAD_FOLDER, filename)

    encode(input_path, payload, output_path)

    return f"✅ Encrypted image created: {filename}"
